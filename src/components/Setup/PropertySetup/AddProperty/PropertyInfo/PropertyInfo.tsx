...
const PropertyInfo = () => {
    ...
    const [propertyInfoDetails, SetPropertyInfoDetails] = React.useState<ProprtyInfo>({
    ...
    const { name, email, space, goodFor, propertyType } = propertyInfoDetails

    const { Cname, CphoneNumber, waNumber } = contactDetails

    const { Oname, OphoneNumber } = ownerDetails

    const { address, city, state, country, latitude, longitude } = locationDetails


    const changeHandler = (e, type) => {
        switch (type) {
            case 'info':
                SetPropertyInfoDetails({ ...propertyInfoDetails, [e.target.name]: e.target.value });
                break;
            case 'contect':
    ...
    const onChange = (option: any, actionMeta: ActionMeta<any>) => {
        ...
        SetPropertyInfoDetails({ ...propertyInfoDetails, [name]: option?.value });
    }
