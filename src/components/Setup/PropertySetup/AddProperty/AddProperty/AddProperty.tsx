...
import { addPropertyData, getPropertyDataById, updatePropertyData } from '../PropertyInfo/propertyInfoSlice'
import { useDispatch } from 'react-redux'
import { AppDispatch } from '../../../../../Redux/Store'
import { ITimezoneOption } from 'react-timezone-select'
import { addTaxConfigDetails, getTaxConfigDetails, updateTaxData, useTaxData } from '../TaxSetup/taxSetupSlice'
...
const AddProperty = (props: AddPropertyProps) => {
	...
	const setTaxData = () => {
		if (taxData[0] && id) {
			setTaxInfo({
				...
				calculationType: taxData[0].calculationType ? taxData[0].calculationType : 0,
				isVatApplicable: taxData[0].isVatApplicable ? taxData[0].isVatApplicable : false,
			})
		}
	}
	...
	//   { skip: !!editPid }
	// );
	const { handleChange, handleSubmit, values, errors, touched, setValues, setFieldValue } = useFormik({
		initialValues,
		validationSchema,
		onSubmit,
	})
		...
		if (id) {
			...
			if (response?.data) {
				setValues({
					...
					propertyType: response?.data?.propertyType ? response?.data?.propertyType : '',
					goodFor: response?.data?.goodFor ? response?.data?.goodFor : '',
					allowedFor: response?.data?.allowedFor ? response?.data?.allowedFor : '',
					area: response?.data?.dimensions?.area ? response?.data?.dimensions?.area : 0,
					unit: response?.data?.dimensions?.unit ? response?.data?.dimensions?.unit : '',
					adults: response?.data?.maxCapacity.adults ? response?.data?.maxCapacity.adults : 0,
					children: response?.data?.maxCapacity.children ? response?.data?.maxCapacity.children : 0,
					Cname: response?.data?.contact.name ? response?.data?.contact.name : '',
					CphoneNumber: response?.data?.contact.phoneNumber ? response?.data?.contact.phoneNumber : '',
					waNumber: response?.data?.contact.waNumber ? response?.data?.contact.waNumber : '',
					Oname: response?.data?.owner.name ? response?.data?.owner.name : '',
					briefDescription: response?.data?.briefDescription ? response?.data?.briefDescription : '',
					longDescription: response?.data?.longDescription ? response?.data?.longDescription : '',
					OphoneNumber: response?.data?.owner.phoneNumber ? response?.data?.owner.phoneNumber : '',
					address: response?.data?.location.address ? response?.data?.location.address : '',
					// city: response?.data?.city,
					Lcity: response?.data?.location.city ? response?.data?.location.city : '',
					state: response?.data?.location.state ? response?.data?.location.state : '',
					zipCode: response?.data?.location.zipCode ? response?.data?.location.zipCode.toString() : '',
					Lcountry: response?.data?.location.country ? response?.data?.location.country : '',
					// district: response?.data?.district,
					// country: response?.data?.country,
					latitude: response?.data?.location.latitude ? response?.data?.location.latitude : '',
					longitude: response?.data?.location.longitude ? response?.data?.location.longitude : '',
					availableForEntireRental: response?.data?.availableForEntireRental
						? response?.data?.availableForEntireRental
						: false,
					strictlyEntireRental: response?.data?.strictlyEntireRental ? response?.data?.strictlyEntireRental : false,
					automaticRoomAssignment: response?.data?.reservationConfig?.automaticRoomAssignment
					...
					emailDisplayName: response?.data?.reservationConfig?.emailDisplayName
						...
						: '',
					replyToEmailAddress: response?.data?.reservationConfig?.replyToEmailAddress
						? response?.data?.reservationConfig?.replyToEmailAddress
						: '',
					sendCCOnAllEmails: response?.data?.reservationConfig?.sendCCOnAllEmails
						? response?.data?.reservationConfig?.sendCCOnAllEmails
						: '',
					setOccupiedRoomToDirty: response?.data?.reservationConfig?.setOccupiedRoomToDirty
						? response?.data?.reservationConfig?.setOccupiedRoomToDirty
						: false,
					allowOverBookingManually: response?.data?.reservationConfig?.allowOverBookingManually
					...
					checkOutTime: response?.data?.checkInCheckOutConfig?.checkOutTime
						...
						: '',
					autoGuestRegistrationCreationDuringCheckIn: response?.data?.checkInCheckOutConfig
						?.autoGuestRegistrationCreationDuringCheckIn
						? response?.data?.checkInCheckOutConfig?.autoGuestRegistrationCreationDuringCheckIn
						: false,
					sendNotificationToConfirmRoomIfDirty: response?.data?.checkInCheckOutConfig
						?.sendNotificationToConfirmRoomIfDirty
						? response?.data?.checkInCheckOutConfig?.sendNotificationToConfirmRoomIfDirty
						: false,
					allowNonZeroBalanceDuringCheckout: response?.data?.checkInCheckOutConfig?.allowNonZeroBalanceDuringCheckout
						? response?.data?.checkInCheckOutConfig?.allowNonZeroBalanceDuringCheckout
						: false,
					allowRefundApplyUponCheckOut: response?.data?.checkInCheckOutConfig?.allowRefundApplyUponCheckOut
						? response?.data?.checkInCheckOutConfig?.allowRefundApplyUponCheckOut
						: false,
					autoRefundApplyUponCheckOut: response?.data?.checkInCheckOutConfig?.autoRefundApplyUponCheckOut
						? response?.data?.checkInCheckOutConfig?.autoRefundApplyUponCheckOut
						: false,
					includeRoomMovesOnArrivalAndDepartureList: response?.data?.checkInCheckOutConfig
						?.includeRoomMovesOnArrivalAndDepartureList
						? response?.data?.checkInCheckOutConfig?.includeRoomMovesOnArrivalAndDepartureList
						: false,
				})
		...
		if (key == 'six') {
			try {
				...
				if (taxData[0]?._id) {
					...
					let response: any = await dispatch(addTaxConfigDetails(payload)).unwrap()
				}
			} catch (err: any) {
				console.log(err, 'err')
			}
			...
			<Card className="card-bg">
				<Card.Body>
					<div className="panel panel-secondary">
						<Tab.Container id="left-tabs-example p-0" onSelect={(k: any) => setKeyValue(k)} activeKey={key}>
                      ...
                      <Nav.Link eventKey="five">
                        <i className="fe fe-user me-1"></i>Policies
									...
									{isTax ? (
										...
										<Button type="submit" style={{ borderRadius: '0px 20px 20px 0px' }} onClick={handleclick}>
											Save
										</Button>
									)}
								</div>
								<div className="tab-content-container">
									<Tab.Content>
										...
										<Tab.Pane eventKey="four">
											<CheckInCheckOut values={values} handleChange={handleChange} setFieldValue={setFieldValue} />
										</Tab.Pane>
										<Tab.Pane eventKey="five"></Tab.Pane>
										<Tab.Pane eventKey="six">
											<TaxSetup initialTaxValuesInfo={taxInfo} setTaxInfo={setTaxInfoValue} />
										</Tab.Pane>
										...
										<Tab.Pane eventKey="eight">
											<Sections sectionArray={sectionArray} setSectionArray={setSectionArray} />
										</Tab.Pane>
									</Tab.Content>
								</div>
							</form>
