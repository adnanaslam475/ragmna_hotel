...
export class BasicSelect extends React.Component<{}, { state: any; options: any }> {
	render() {
		return <Select options={options} placeholder="choose one" classNamePrefix="Select" />
	}
}
...
export class BasicSelectSearch extends React.Component<{}, { options: any }> {
	render() {
		return (
			<Select className="basic-single" classNamePrefix="Select" defaultValue={Data[0]} name="color" options={Data} />
		)
	}
}
...
export function BasicStyle() {
	...
	return (
		<div>
			...
			<Button className="btn-pill" size="sm" onClick={() => setShowColorPicker((showColorPicker) => !showColorPicker)}>
...
export const CountrySelector = () => {
	return (
		<div>
			<PhoneInput country={'us'} value="1425652" onChange={(phone) => console.log({ phone })} />
		</div>
	)
}
